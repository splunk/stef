...
type LineArray struct {
	elems       []*Line // all pointers are non-nil
	initedCount int
...
func copyToNewLineArray(dst *LineArray, src *LineArray, allocators *Allocators) {
	...
	for j := 0; j < len(dst.elems); j++ {
		if src.elems[j].canBeShared() {
			...
			allocators.allocSizeChecker.AddAllocSize(uint(unsafe.Sizeof(Line{})))
...
// specified schema.
func (a *LineArray) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	if random.IntN(20) == 0 {
...
func (d *LineArrayDecoder) Decode(dst *LineArray) error {
	...
	if lenDelta > 0 {
		if err := d.allocators.allocSizeChecker.PrepAllocSizeN(uint(lenDelta), uint(unsafe.Sizeof(dst.elems[0])+unsafe.Sizeof(Line{}))); err != nil {
			return err
		}
	}
