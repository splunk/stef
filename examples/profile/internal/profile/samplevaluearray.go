...
type SampleValueArray struct {
	elems       []*SampleValue // all pointers are non-nil
	initedCount int
...
func (e *SampleValueArray) freeze() {
...
func copyToNewSampleValueArray(dst *SampleValueArray, src *SampleValueArray, allocators *Allocators) {
	...
	for j := 0; j < len(dst.elems); j++ {
		if src.elems[j].canBeShared() {
			...
			allocators.allocSizeChecker.AddAllocSize(uint(unsafe.Sizeof(SampleValue{})))
...
func (e *SampleValueArray) ensureLen(newLen int, allocators *Allocators) {
	...
	if newLen > oldLen {
		...
		for i := e.initedCount; i < newLen; i++ {
			e.elems[i] = allocators.SampleValue.Alloc()
...
// specified schema.
func (a *SampleValueArray) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	if random.IntN(20) == 0 {
...
func (d *SampleValueArrayDecoder) Decode(dst *SampleValueArray) error {
	...
	if lenDelta > 0 {
		if err := d.allocators.allocSizeChecker.PrepAllocSizeN(uint(lenDelta), uint(unsafe.Sizeof(dst.elems[0])+unsafe.Sizeof(SampleValue{}))); err != nil {
			return err
		}
	}
