...
func (s *Sample) reset() {
	if s.modifiedFields.isFrozen() {
		panic("cannot modify frozen Sample")
	}
	s.metadata.reset()
	s.locations.reset()
	s.values.reset()
...
func (s *Sample) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	...
	if random.IntN(randRange) == 0 {
		s.metadata.mutateRandom(random, schem, limiter)
	}
	if fieldCount <= 1 {
	...
	if random.IntN(randRange) == 0 {
		s.locations.mutateRandom(random, schem, limiter)
	}
	if fieldCount <= 2 {
	...
	// Maybe mutate Values
	if random.IntN(randRange) == 0 {
		s.values.mutateRandom(random, schem, limiter)
	}
	if fieldCount <= 3 {
	...
	// Maybe mutate Labels
	if random.IntN(randRange) == 0 {
		s.labels.mutateRandom(random, schem, limiter)
	}
}
...
func (d *SampleDecoder) tryAllocSize(size int) error {
