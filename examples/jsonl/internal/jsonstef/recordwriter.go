...
func NewRecordWriter(dst pkg.ChunkWriter, opts pkg.WriterOptions) (*RecordWriter, error) {
	writer := &RecordWriter{
		dst:  dst,
		opts: opts,
	}
...
func (f *RecordWriter) writeFixedHeader() error {
	var hdrTail []byte
...
func (w *RecordWriter) writeVarHeader() error {
	// Serialize varheader
	hdr := pkg.VarHeader{}
	if w.opts.IncludeDescriptor {
		if w.opts.Schema != nil {
			...
			hdr.SchemaWireBytes = []byte(wireSchemaRecord)
		}
	}
...
func (w *RecordWriter) Write() error {
...
func (w *RecordWriter) RecordCount() uint64 {
	return w.recordCount
}

func (w *RecordWriter) restartFrame(nextFrameFlags pkg.FrameFlags) error {
