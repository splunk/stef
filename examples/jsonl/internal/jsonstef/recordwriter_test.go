...
func genRecordRecords(random *rand.Rand, schem *schema.Schema, recCount int) (records []Record) {
	var record Record
	record.Init()
	...
	for _, opt := range testWriterOpts {
		t.Run(
			"", func(t *testing.T) {
				...
				records := genRecordRecords(random, schem, 1000)
				// Write the records
...
func TestRecordWriteReadLong(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		records := genRecordRecords(random, schem, 1+random.IntN(1000))
