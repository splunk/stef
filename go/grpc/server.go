...
type ServerSettings struct {
	// Logger is used for logging server events and errors. If nil, a no-op logger will be used.
	// The logger should be safe for concurrent use across multiple goroutines.
	Logger types.Logger

	// ServerSchema defines the wire schema that the server will use for STEF message validation
	// and processing. This schema must be compatible with the data being received from clients.
	ServerSchema *schema.WireSchema

	// MaxDictBytes sets the maximum size in bytes for dictionary data that the server will accept.
	// This limit helps prevent memory exhaustion.
