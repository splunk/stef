...
func quantilesFromStef(src pmetric.SummaryDataPointValueAtQuantileSlice, dst *oteltef.QuantileValueArray) {
	dst.EnsureLen(src.Len())
	for i := 0; i < src.Len(); i++ {
		dst.At(i).SetQuantile(src.At(i).Quantile())
		dst.At(i).SetValue(src.At(i).Value())
	}
}
