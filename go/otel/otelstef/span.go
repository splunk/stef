...
func (s *Span) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	...
	// Maybe mutate Events
	if random.IntN(randRange) == 0 {
		s.events.mutateRandom(random, schem, limiter)
	}
	if fieldCount <= 12 {
	...
	// Maybe mutate Links
	if random.IntN(randRange) == 0 {
		s.links.mutateRandom(random, schem, limiter)
	}
	if fieldCount <= 13 {
	...
	// Maybe mutate Status
	if random.IntN(randRange) == 0 {
		s.status.mutateRandom(random, schem, limiter)
	}
}
...
func (d *SpanDecoder) tryAllocSize(size int) error {
