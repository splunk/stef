...
// EnsureLen ensures the length of the array is equal to newLen.
// It will grow or shrink the array if needed.
func (e *EventArray) ensureLen(newLen int, allocators *Allocators) error {
	oldLen := len(e.elems)
	if newLen > oldLen {
		...
		allocators.addAllocSize(lenDelta * int(unsafe.Sizeof(e.elems[0])+unsafe.Sizeof(Event{})))
		if allocators.isOverLimit() {
			return pkg.ErrRecordAllocLimitExceeded
		}

		// Grow the array
		e.elems = append(e.elems, make([]*Event, lenDelta)...)
		e.markModified()
