...
type EventArray struct {
	elems       []*Event // all pointers are non-nil
	initedCount int
...
func copyToNewEventArray(dst *EventArray, src *EventArray, allocators *Allocators) {
	...
	for j := 0; j < len(dst.elems); j++ {
		if src.elems[j].canBeShared() {
			...
			allocators.allocSizeChecker.AddAllocSize(uint(unsafe.Sizeof(Event{})))
...
func (e *EventArray) ensureLen(newLen int, allocators *Allocators) {
	...
	if newLen > oldLen {
		...
		for i := e.initedCount; i < newLen; i++ {
			e.elems[i] = allocators.Event.Alloc()
...
func (d *EventArrayDecoder) Decode(dst *EventArray) error {
	...
	if lenDelta > 0 {
		if err := d.allocators.allocSizeChecker.PrepAllocSizeN(uint(lenDelta), uint(unsafe.Sizeof(dst.elems[0])+unsafe.Sizeof(Event{}))); err != nil {
			return err
		}
	}
