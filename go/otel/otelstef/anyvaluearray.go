...
type AnyValueArray struct {
	elems       []*AnyValue // all pointers are non-nil
	initedCount int
...
func (e *AnyValueArray) ensureLen(newLen int, allocators *Allocators) {
	...
	if newLen > oldLen {
		...
		for i := e.initedCount; i < newLen; i++ {
			e.elems[i] = allocators.AnyValue.Alloc()
...
// specified schema.
func (a *AnyValueArray) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	if random.IntN(20) == 0 {
...
func (d *AnyValueArrayDecoder) Decode(dst *AnyValueArray) error {
	...
	if lenDelta > 0 {
		if err := d.allocators.allocSizeChecker.PrepAllocSizeN(uint(lenDelta), uint(unsafe.Sizeof(dst.elems[0])+unsafe.Sizeof(AnyValue{}))); err != nil {
			return err
		}
	}
