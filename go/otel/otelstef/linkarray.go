...
// EnsureLen ensures the length of the array is equal to newLen.
// It will grow or shrink the array if needed.
func (e *LinkArray) ensureLen(newLen int, allocators *Allocators) error {
	oldLen := len(e.elems)
	if newLen > oldLen {
		...
		allocators.addAllocSize(lenDelta * int(unsafe.Sizeof(e.elems[0])+unsafe.Sizeof(Link{})))
		if allocators.isOverLimit() {
			return pkg.ErrRecordAllocLimitExceeded
		}

		// Grow the array
		e.elems = append(e.elems, make([]*Link, lenDelta)...)
		e.markModified()
...
// specified schema.
func (a *LinkArray) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	if random.IntN(20) == 0 {
