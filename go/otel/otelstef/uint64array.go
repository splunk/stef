...
// EnsureLen ensures the length of the array is equal to newLen.
// It will grow or shrink the array if needed.
func (e *Uint64Array) ensureLen(newLen int, allocators *Allocators) error {
	oldLen := len(e.elems)
	if newLen > oldLen {
		...
		// Grow the array
		e.elems = append(e.elems, make([]uint64, lenDelta)...)
		e.markModified()
...
// specified schema.
func (a *Uint64Array) mutateRandom(random *rand.Rand, schem *schema.Schema, limiter *mutateRandomLimiter) {
	if random.IntN(20) == 0 {
