...
func testMetricsWriteReadSeed(t *testing.T, seed uint64) (retVal bool) {
	retVal = true
	...
	for _, opt := range testWriterOpts {
		t.Run(
			"", func(t *testing.T) {
				...
				writer, err := NewMetricsWriter(buf, optCpy)
				require.NoError(t, err, "seed %v", seed)
...
func TestMetricsWriteRead(t *testing.T) {
	seedFileName := "../../../seeds/otelstef_Metrics_seeds.txt"
	...
	for _, seedStr := range seeds {
		...
		testMetricsWriteReadSeed(t, seed)
	}
...
func TestMetricsWriteReadLong(t *testing.T) {
	...
	for i := 0; i < iterations; i++ {
		records := genMetricsRecords(random, schem, 1+random.IntN(1000))
...
func FuzzMetricsReader(f *testing.F) {
	...
	f.Fuzz(
		func(t *testing.T, data []byte) {
			reader, err := NewMetricsReader(bytes.NewBuffer(data))
			if err != nil {
				return
			}
			for {
