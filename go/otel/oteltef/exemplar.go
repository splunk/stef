...
func (e *ExemplarEncoder) Init(state *WriterState, columns *pkg.WriteColumnSet) error {
	...
	if state.OverrideSchema != nil {
		overrideSchema, ok := state.OverrideSchema.Structs["Exemplar"]
	...
	if err := e.spanIDEncoder.Init(&state.Span, e.limiter, columns.AddSubColumn()); err != nil {
		return err
	}
	if e.fieldCount <= 3 {
		return nil // TraceID and subsequent fields are skipped.
	}
	if err := e.traceIDEncoder.Init(&state.Trace, e.limiter, columns.AddSubColumn()); err != nil {
		return err
	}
	if e.fieldCount <= 4 {
...
func (d *ExemplarDecoder) Init(state *ReaderState, columns *pkg.ReadColumnSet) error {
	...
	err = d.spanIDDecoder.Init(&state.Span, columns.AddSubColumn())
	if err != nil {
		return err
	}
	if d.fieldCount <= 3 {
	...
	err = d.traceIDDecoder.Init(&state.Trace, columns.AddSubColumn())
	if err != nil {
		return err
	}
	if d.fieldCount <= 4 {
