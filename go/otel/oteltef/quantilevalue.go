...
func (e *QuantileValue) IsEqual(val *QuantileValue) bool {
	if !pkg.Float64Equal(e.quantile, val.quantile) {
		return false
	}
	if !pkg.Float64Equal(e.value, val.value) {
		return false
	}
...
func (e *QuantileValueEncoder) Init(state *WriterState, columns *pkg.WriteColumnSet) error {
	...
	if err := e.quantileEncoder.Init(e.limiter, columns.AddSubColumn()); err != nil {
		return err
	}
	if e.fieldCount <= 1 {
		return nil // Value and subsequent fields are skipped.
	}
	if err := e.valueEncoder.Init(e.limiter, columns.AddSubColumn()); err != nil {
		return err
	}
...
func (e *QuantileValueEncoder) CollectColumns(columnSet *pkg.WriteColumnSet) {
	...
	e.quantileEncoder.CollectColumns(columnSet.At(0))
	if e.fieldCount <= 1 {
