...
func diffData(rootStruct, fname1, fname2 string) error {
	...
	case "Spans":
		reader1, err := NewSpansReader(file1)
		if err != nil {
			return fmt.Errorf("Cannot open %s: %v", fname1, err)
		}

		reader2, err := NewSpansReader(file2)
		if err != nil {
			return fmt.Errorf("Cannot open %s: %v", fname1, err)
		}
...
func copyData(rootStruct, fname1, fname2 string) (retErr error) {
	...
	case "Spans":
		reader, err := NewSpansReader(sourceFile)
		if err != nil {
			return fmt.Errorf("Cannot create reader for %s: %v", fname1, err)
		}

		writer, err := NewSpansWriter(dst, pkg.WriterOptions{})
		if err != nil {
	...
	case "Metrics":
		reader, err := NewMetricsReader(sourceFile)
		if err != nil {
			return fmt.Errorf("Cannot create reader for %s: %v", fname1, err)
		}
