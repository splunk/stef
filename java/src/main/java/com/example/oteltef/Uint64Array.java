...
public class Uint64Array {
    ...
    Uint64Array(ModifiedFields parentModifiedFields, long parentModifiedBit) {
    ...
    public Uint64Array clone() {
        Uint64Array clone = new Uint64Array();
        clone.copyFrom(this);
        return clone;
    }
    ...
    boolean markDiffModified(Uint64Array v) {
        ...
        int i=0;
        for (; i < minLen; i++) {
            if (!Types.Uint64Equal(this.elems[i], v.elems[i])) {
                modified = true;
            }
        }
    ...
    public void copyFrom(Uint64Array src) {
    ...
    public boolean equals(Uint64Array val) {
        if (elemsLen != val.elemsLen) {
            return false;
        }
        for (int i = 0; i < elemsLen; i++) {
            
            if (!Types.Uint64Equal(elems[i], val.elems[i])) {
                return false;
            }
    ...
    public static int compare(Uint64Array left, Uint64Array right) {
        int c = left.elemsLen - right.elemsLen;
        if (c != 0) {
            return c;
        }
        for (int i = 0; i < left.elemsLen; i++) {
            int fc = Types.Uint64Compare(left.elems[i], right.elems[i]);
            if (fc < 0) {
                return -1;
            }
            if (fc > 0) {
    ...
    void mutateRandom(Random random) {
        ...
        for (int i = 0; i < elemsLen; i++) {
            if (random.nextInt(2 * elemsLen) == 0) {
                ...
                var v = Types.Uint64Random(random);
