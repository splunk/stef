...
public class QuantileValueArray {
    QuantileValue[] elems = new QuantileValue[0];
    ...
    QuantileValueArray(ModifiedFields parentModifiedFields, long parentModifiedBit) {
    ...
    public QuantileValueArray clone() {
        QuantileValueArray clone = new QuantileValueArray();
        clone.copyFrom(this);
        return clone;
    }
    ...
    public void append(QuantileValue val) {
    ...
    boolean markDiffModified(QuantileValueArray v) {
    ...
    public void copyFrom(QuantileValueArray src) {
        ...
        if (minLen < elemsLen) {
            ...
            for (int j=0; j < addLen; j++) {
                // Init the element.
                elems[i+j] = new QuantileValue(parentModifiedFields, parentModifiedBit);
                // Copy the element.
    ...
    void ensureElems(int newLen) {
        if (elems.length < newLen) {
            ...
            QuantileValue[] newElems = new QuantileValue[allocLen];
    ...
    public void ensureLen(int newLen) {
        ...
        if (newLen > oldLen) {
            ...
            for (int i = oldLen; i < newLen; i++) {
                ...
                elems[i] = new QuantileValue(parentModifiedFields, parentModifiedBit);
            }
    ...
    public boolean equals(QuantileValueArray val) {
    ...
    public static int compare(QuantileValueArray left, QuantileValueArray right) {
        int c = left.elemsLen - right.elemsLen;
        if (c != 0) {
            return c;
        }
        for (int i = 0; i < left.elemsLen; i++) {
            int fc = QuantileValue.compare(left.elems[i], right.elems[i]);
            if (fc < 0) {
                return -1;
            }
            if (fc > 0) {
