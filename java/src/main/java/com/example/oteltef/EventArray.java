...
public class EventArray {
    ...
    public EventArray clone() {
        EventArray clone = new EventArray();
        clone.copyFrom(this);
        return clone;
    }
    ...
    boolean markDiffModified(EventArray v) {
    ...
    public void copyFrom(EventArray src) {
        ...
        if (minLen < elemsLen) {
            ...
            for (int j=0; j < addLen; j++) {
                // Init the element.
                elems[i+j] = new Event(parentModifiedFields, parentModifiedBit);
                // Copy the element.
    ...
    void ensureElems(int newLen) {
        if (elems.length < newLen) {
            ...
            Event[] newElems = new Event[allocLen];
    ...
    public boolean equals(EventArray val) {
    ...
    public static int compare(EventArray left, EventArray right) {
        int c = left.elemsLen - right.elemsLen;
        if (c != 0) {
            return c;
        }
        for (int i = 0; i < left.elemsLen; i++) {
            int fc = Event.compare(left.elems[i], right.elems[i]);
            if (fc < 0) {
                return -1;
            }
            if (fc > 0) {
