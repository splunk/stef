...
const reconnectPeriod = 10 * time.Minute
...
func (s *stefExporter) Start(ctx context.Context, host component.Host) error {
	...
	s.connMan = internal.NewConnManager(s.set.Logger, connCreator, 1, flushPeriod, reconnectPeriod)
	s.connMan.Start()

	s.sync2Async = internal.NewSync2Async(s.set.Logger, s.cfg.NumConsumers, s.exportMetricsAsync)
...
func (s *stefExporter) exportMetrics(ctx context.Context, data pmetric.Metrics) error {
	return s.sync2Async.Sync(ctx, data)
}

func (s *stefExporter) exportMetricsAsync(
	ctx context.Context,
	data any,
